...
class BinaanController extends Controller
{
    ...
    public function index(DataTables $dataTables, Request $request)
    {
        if(request()->ajax())
        {
            if($request->waktu)
            {
                ...
                if($request->category == 'ke_litmas')
                {
                    $binaan = $binaan->filter(function ($value, $key) {
                        return $value->proses[5]->pivot->tanggal == null;
                    });
    
                    if($request->waktu == 7)
                    {
                        $model = $binaan->filter(function ($value, $key) {
                            $tanggal = $value->proses[4]->pivot->tanggal;
                            $tanggal = date_create($tanggal);
                            $interval = $tanggal->diff(date_create());      
                            return $interval->d > 7;
                        });    
                    }
                    elseif($request->waktu == 14)
                    {
                        $model = $binaan->filter(function ($value, $key) {
                            $tanggal = $value->proses[4]->pivot->tanggal;
                            $tanggal = date_create($tanggal);
                            $interval = $tanggal->diff(date_create());      
                            return $interval->d > 14;
                        });    
                    }    
                }
                elseif($request->category == 'interview')
                {
                    $binaan = $binaan->filter(function ($value, $key) {
                        return $value->proses[6]->pivot->tanggal == null;
                    });
    
                    if($request->waktu == 7)
                    {
                        $model = $binaan->filter(function ($value, $key) {
                            $tanggal = $value->proses[5]->pivot->tanggal;
                            $tanggal = date_create($tanggal);
                            $interval = $tanggal->diff(date_create());      
                            return $interval->d > 7;
                        });    
        ...
        $binaan = Binaan::all();     
        $binaan_filter = $binaan->filter(function ($value, $key) {
            $tanggal = $value->proses[5]->pivot->tanggal;
            $tanggal = date_create($tanggal);
            $interval = $tanggal->diff(date_create());      
            return $interval->d > 7;
    ...
    {
        $binaans = Binaan::all();
        $proses_per_binaan = array();
        foreach ($binaans as $key => $binaan) {
            $proses_per_binaan[] = $binaan->proses()->wherePivot('tanggal', '!=', null)->count();
        }
        $proses_per_binaan = collect($proses_per_binaan);
        $proses = $proses_per_binaan->countBy();
        return view('home', compact(['binaans', 'proses']));
    }
